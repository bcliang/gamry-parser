...
class TestChronoAmperometry(unittest.TestCase):
    ...
    def test_use_datetime(self):
        gp = parser.ChronoAmperometry(
            filename='tests/chronoa_data.dta', to_timestamp=False)
        gp.load()
        ...
        gp = parser.ChronoAmperometry(
            filename='tests/chronoa_data.dta', to_timestamp=True)
        gp.load()
        curve = gp.get_curve_data()
        # 'T' should return datetime objects
        self.assertEqual(curve['T'][0], pd.to_datetime('3/10/2019 12:00:00'))
        self.assertEqual(curve['T'].iloc[-1],
                         pd.to_datetime('3/10/2019 12:04:30'))
